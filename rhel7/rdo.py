...
def get_token():
    ...
    host = testbed.env['roledefs']['openstack'][0].split('@')[1]
    password = getattr(env, 'password', 'c0ntrail123')
    ssh.connect(host, username = 'root', password = password)

    #stdin, stdout, stderr = ssh.exec_command('cat /etc/keystone/keystone.conf')
    #print stdout

    sftp = ssh.open_sftp()
    sftp.get('/etc/keystone/keystone.conf', '/opt/contrail/keystone.conf')
    ssh.close()
